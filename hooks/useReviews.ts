...
export const useReviews = (productId?: string) => {
  ...
  const { data: reviews = [], isLoading: isLoadingReviews, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      // সঠিক ইউআরএল: http://localhost:8080/api/v1/reviews/product/YOUR_PRODUCT_ID
...
const { mutateAsync: createReview, isPending: isCreatingReview } = useMutation({
  ...
  const { mutateAsync: updateReviewStatus, isPending: isUpdatingStatus } = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: "Active" | "Inactive" }) => {
      const response = await axios.patch(`${API_BASE_URL}/reviews/${id}/status`, { status });
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["reviews"] });
