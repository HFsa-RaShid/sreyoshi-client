...
export default function DashboardHomePage() {
  ...
  // পেজ চেঞ্জ করে এখানে আসলে ক্যাশ ডাটা রিভ্যালিডেট করার জন্য
  useEffect(() => {
  ...
  // 🎯 [THE FIX]: ডাটা রিয়েলটাইমে না আসা পর্যন্ত স্কেলিটন লক করে রাখার ম্যাজিক শর্ত
  const isDataMissing = isUserLoading || isOrdersLoading || !rawOrders || !userId;

  // স্ট্যাটস ও চার্ট ক্যালকুলেশন
  const stats = useMemo(() => {
    if (isDataMissing || !Array.isArray(rawOrders)) {
      return {
  ...
  }, [rawOrders, userId, isDataMissing]);
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in duration-200 font-sans text-[#1E1E1E]">
      ...
      {/* ২. ৪টি মেইন কার্ডের গ্রিড (স্কেলিটন সহ) */}
      ...
      {/* ৪. সাম্প্রতিক ৩টি অর্ডারের লিস্ট (স্কেলিটন রো সহ) */}
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-2xs">
        ...
        <div className="space-y-3">
          {isDataMissing ? (
            // নিখুঁত ৩টি লাইনের সুন্দর ব্লিঙ্কিং স্কেলিটন লোডার
            [1, 2, 3].map((n) => (
            ...
            stats.recentOrders.map((order: any, idx: number) => {
              const rawDate = order.createdAt?.$date || order.createdAt;
              const dateStr = rawDate ? new Date(rawDate).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "N/A";
                ...
                <div key={order._id || idx} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 border border-gray-50 rounded-xl hover:bg-gray-50/50 transition-colors">
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 border-t sm:border-t-0 pt-2 sm:pt-0 border-gray-100">
                    <div className="sm:text-right">
                      ...
                      <span className={`inline-block text-[10px] font-bold capitalize mt-0.5 ${status.toLowerCase() === "delivered" ? "text-emerald-600" : status.toLowerCase() === "cancelled" ? "text-red-500" : "text-amber-500"}`}>• {status}</span>
                    </div>
