...
export default function DashboardHomePage() {
  ...
  // ২. শুধুমাত্র কারেন্ট ইউজারের অর্ডার ফিল্টার এবং ক্যালকুলেশন (useMemo দিয়ে অপ্টিমাইজড)
  const stats = useMemo(() => {
    const myOrders = rawOrders.filter((order: any) => getSafeId(order.user) === userId);
    ...
    // চার্টের জন্য লাস্ট ৭ দিনের ডেটা স্ট্রাকচার রেডি করা
    ...
    myOrders.forEach((order: any) => {
      ...
      // সফল বা ডেলিভার্ড অর্ডারের টোটাল খরচ যোগ করা
      if (status !== "cancelled" && status !== "canceled") {
        totalSpent += order.totalPrice || 0;
      }

      // অর্ডার ডেট ট্র্যাকিং ফর চার্ট
    ...
    ].filter(item => item.value > 0); // শুধু ভ্যালু থাকা আইটেম দেখাবে

    // সাম্প্রতিক ৩টি অর্ডার বের করা
  ...
  if (isLoading) {
    return (
      <div className="h-[400px] flex flex-col items-center justify-center gap-3 bg-white rounded-2xl border border-gray-100">
        <div className="w-8 h-8 border-4 border-[#4E612B] border-t-transparent rounded-full animate-spin" />
        <span className="text-xs text-gray-400 font-medium animate-pulse">Loading Analytics Dashboard...</span>
      </div>
    );
  }
  ...
  if (!userId) {
    return (
      <div className="h-[300px] flex items-center justify-center bg-white rounded-2xl border border-gray-100 p-6 text-center">
        <p className="text-sm font-medium text-gray-500">Please log in to synchronize your dynamic analytics dashboard.</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in duration-300 font-sans text-[#1E1E1E]">
      ...
      {/* 📊 ২. ৪টি স্মার্ট স্ট্যাটস কার্ড গ্রিড */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { name: "Active Orders", val: stats.pendingCount, sub: "In processing", icon: Clock, color: "bg-amber-50 text-amber-600 border-amber-100" },
      ...
      {/* 📈 ৩. চার্টস ও অ্যানালিটিক্স সেকশন (রেসপন্সিভ গ্রাফ) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        
        {/* লাইভ লাইন/এরিয়া চার্ট */}
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-sm font-bold text-[#0F1E29] flex items-center gap-1.5">
                <TrendingUp size={16} className="text-[#4E612B]" /> Weekly Expense Analytics
              </h3>
      ...
      {/* 🛒 ৪. সাম্প্রতিক অর্ডার লগ টেবিল/লিস্ট */}
                ...
                <div key={order._id || idx} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 border border-gray-50 rounded-xl hover:bg-gray-50/50 transition-colors">
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 border-t sm:border-t-0 pt-2 sm:pt-0 border-gray-100">
                    <div className="sm:text-right">
                      ...
                      <span className={`inline-block text-[10px] font-bold capitalize mt-0.5 ${
                        status === 'Delivered' ? 'text-emerald-600' : status === 'Cancelled' ? 'text-red-500' : 'text-amber-500'
                      }`}>
                        • {status}
                      </span>
                    </div>
...
// ছোট হেল্পার আইকন (ChevronRight মডিউল সেভ করার জন্য)
