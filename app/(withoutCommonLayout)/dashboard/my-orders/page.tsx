...
// import { Eye, Truck, ShoppingCart } from "lucide-react";
...
const getSafeId = (productField: any): string => {
  ...
  if (typeof productField === "object") {
    if (productField.$oid) return productField.$oid;
    if (productField._id) {
      return typeof productField._id === "object" && productField._id.$oid ? productField._id.$oid : String(productField._id);
    }
  }
  return String(productField);
};

// ... (IndividualProductImage এবং OrderItemTitle কম্পোনেন্ট আগের মতোই থাকবে)
...
export default function MyOrdersPage() {
  ...
  // 🎯 নতুন স্টেট: ইউজার details দেখতে চায় নাকি track করতে চায়
  const [viewMode, setViewMode] = useState<"details" | "track" | null>(null);
  ...
  const filterOptions: OrderFilter[] = ["All", "Pending", "Confirmed", "Cancelled"];

  const getStatusBadgeStyle = (status: string) => {
    if (status === "Pending") return "bg-[#FFF9E6] text-[#D9A700]";
    if (status === "Confirmed") return "bg-[#EBF5EE] text-[#2D4A3E]";
    return "bg-[#FCE8E6] text-[#D93025]";
  };

  // ─── 🎯 রাউটিং/ভিউ হ্যান্ডলিং কন্ডিশন ───
  if (activeOrderId && viewMode === "details") {
    const selectedOrder = rawOrders.find((o: any) => getSafeId(o._id) === activeOrderId);
    return (
      <OrderDetailsPage
        orderData={selectedOrder}
        onBack={() => { setActiveOrderId(null); setViewMode(null); }}
        allProducts={allProducts}
      />
    );
  }

  if (activeOrderId && viewMode === "track") {
    const selectedOrder = rawOrders.find((o: any) => getSafeId(o._id) === activeOrderId);
    return (
      <TrackOrderPage
  ...
  if (isPageLoading) {
    return <div className="p-6 text-center">Loading orders...</div>; // (আপনার আগের স্কেলিটন লোডারটি এখানে বসাতে পারেন)
  }

  const myFilteredOnlyOrders = rawOrders.filter((order: any) => getSafeId(order.user) === userId);
  const sortedOrders = [...myFilteredOnlyOrders].sort((a: any, b: any) => new Date(b.createdAt?.$date || b.createdAt).getTime() - new Date(a.createdAt?.$date || a.createdAt).getTime());
  const filteredOrders = sortedOrders.filter((order: any) => {
    if (activeFilter === "All") return true;
    const status = (order.orderStatus || "Pending").toLowerCase();
  ...
  return (
    <div className="w-full bg-[#FAFAFA] min-h-screen p-3 md:p-6 text-[#1E1E1E]">
      <div className="max-w-5xl mx-auto">
        {/* Header content and filters code here... */}
        ...
        <div className="space-y-4 mt-6">
          {currentOrders.map((order: any, idx: number) => {
            ...
            return (
              <div key={orderStringId || idx} className="p-4 md:p-5 border border-gray-100 bg-white rounded-2xl flex flex-col gap-4">
                {/* Card contents... */}
                
                <div className="flex justify-between items-center border-t pt-3">
                  <span className={`px-2.5 py-0.5 rounded-lg text-xs font-bold ${getStatusBadgeStyle(currentStatus)}`}>
                    • {currentStatus}
                  </span>
                  ...
                  <div className="flex items-center gap-2">
                    {/* Details বাটন */}
                    <button
                      onClick={() => { setActiveOrderId(orderStringId); setViewMode("details"); }}
                      className="flex items-center gap-1 px-4 py-2 rounded-xl border text-xs font-bold text-[#555555] bg-white cursor-pointer"
                    >
                      <Eye size={14} /> Details
                    </button>
                    ...
                    {["Pending", "Confirmed"].includes(currentStatus) ? (
                      <button
                        onClick={() => { setActiveOrderId(orderStringId); setViewMode("track"); }}
                        className="flex items-center gap-1 px-4 py-2 rounded-xl bg-[#4E612B] text-xs font-bold text-white cursor-pointer"
                      >
                        <Truck size={14} /> Track Order
                      </button>
                    ) : (
                      <button className="flex items-center gap-1 px-4 py-2 rounded-xl border text-xs font-bold text-[#4E612B] bg-white">
