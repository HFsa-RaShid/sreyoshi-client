...
type OrderFilter = "All" | "Pending" | "Packed" | "Shipped" | "Delivered" | "Cancelled" ;
...
function IndividualProductImage({ itemField, allProducts }: { itemField: any; allProducts: any[] }) {
  ...
  if (isLoading) {
    return <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl bg-gray-100 animate-pulse shrink-0" />;
  }
...
function OrderItemTitle({ orderItems, allProducts }: { orderItems: any[]; allProducts: any[] }) {
  ...
  return <p className="text-[14px] md:text-[16px] font-bold text-[#1E1E1E] truncate">{displayTitle}</p>;
}
...
export default function MyOrdersPage() {
  ...
  const filterOptions: OrderFilter[] = ["All", "Pending", "Packed", "Shipped", "Delivered", "Cancelled"];

  const getStatusBadgeStyle = (status: string) => {
    switch (status) {
      case "Pending": case "Processing": return "bg-[#FFF9E6] text-[#D9A700]";
      case "Packed": return "bg-[#F3E8FF] text-[#6B21A8]";
      case "Shipped": case "Out for delivery": return "bg-[#EBF3FF] text-[#0066FF]";
      case "Delivered": return "bg-[#EBF5EE] text-[#2D4A3E]";
      case "Cancelled": case "Canceled": return "bg-[#FCE8E6] text-[#D93025]";
  ...
  if (isLoading) {
    return (
      <div className="min-h-[350px] md:min-h-[450px] flex flex-col items-center justify-center gap-2 bg-white rounded-2xl border border-gray-100 p-4">
        ...
        <p className="text-[12px] md:text-[13px] text-gray-400">Loading your secure dashboard...</p>
      </div>
    );
  }
  ...
  if (!userId) {
    return (
      <div className="min-h-[350px] flex flex-col items-center justify-center bg-white rounded-2xl p-4">
        <p className="text-[13px] md:text-[14px] font-medium text-gray-500">Please log in to view your orders.</p>
      </div>
    );
  }
  ...
  const filteredOrders = sortedOrders.filter((order: any) => {
    ...
    if (activeFilter === "Cancelled") return status === "cancelled" || status === "canceled";
    if (activeFilter === "Pending") return status === "pending" || status === "processing";
    if (activeFilter === "Shipped") return status === "shipped" || status === "out for delivery";
                ...
                <div key={orderStringId || idx} className="p-4 md:p-5 border border-gray-100 bg-white rounded-2xl flex flex-col gap-4 shadow-2xs">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 border-b border-gray-100 pb-3">
                    <div className="grid grid-cols-2 gap-y-3 gap-x-6 sm:flex sm:items-center sm:gap-10 md:gap-14 text-[12px] md:text-[13px]">
                      ...
                      <div>
                        <p className="text-[11px] text-gray-400 font-medium uppercase tracking-wider">Placed On</p>
                        <p className="font-medium text-[#555555] mt-0.5">{orderDate}</p>
                      </div>
                      <div>
                        <p className="text-[11px] text-gray-400 font-medium uppercase tracking-wider">Payment</p>
                        <p className="font-medium text-[#555555] mt-0.5">{order.paymentMethod || "COD"}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start sm:items-center gap-3 md:gap-4 flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        {items.length > 2 && (
                          <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl border border-gray-100 bg-[#F4F4F4] flex items-center justify-center shrink-0">
                            <p className="text-[12px] md:text-[13px] font-bold text-[#555555]">+{items.length - 2}</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="space-y-1 min-w-0 flex-1 pt-0.5 sm:pt-0">
                        ...
                        <p className="text-[12px] md:text-[13px] text-gray-400 truncate flex items-center gap-1">
                          ...
                          <span className="truncate">{order.shippingAddress?.address || "Dhaka, Bangladesh"}</span>
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-3 md:gap-4 shrink-0 border-t md:border-t-0 pt-3 md:pt-0 border-gray-50 mt-1 md:mt-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {["Pending", "Packed", "Shipped", "Out for delivery", "Processing"].includes(currentStatus) ? (
                          <button onClick={() => setActiveOrderId(orderStringId)} className="flex items-center gap-1 px-3 py-1.5 md:px-4 md:py-2 rounded-xl bg-[#4E612B] text-[12px] md:text-[13px] font-bold text-white hover:bg-[#3d4d22] cursor-pointer shadow-sm">
                            <Truck size={14} /> Track
                          </button>
                        ) : (
                          <button className="flex items-center gap-1 px-3 py-1.5 md:px-4 md:py-2 rounded-xl border border-gray-200 text-[12px] md:text-[13px] font-bold text-[#4E612B] hover:bg-gray-50 cursor-pointer shadow-2xs bg-white">
                            <ShoppingCart size={13} /> <span className="hidden xs:inline">Again</span>
                          </button>
                        )}
                      </div>
                    </div>
          ...
          <div className="flex items-center justify-between mt-6 pt-2">
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: totalPages }, (_, i) => (
                <button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`w-8 h-8 md:w-9 md:h-9 rounded-xl text-[12px] md:text-[13px] font-extrabold transition-all cursor-pointer ${currentPage === i + 1 ? "bg-[#4E612B] text-white shadow-sm" : "bg-white text-gray-500 border border-gray-200 hover:bg-gray-50 shadow-2xs"}`}>
                  {i + 1}
                </button>
              ))}
            </div>
            <button onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="px-3 py-1.5 md:px-4 md:py-2 rounded-xl border border-gray-200 text-[12px] md:text-[13px] font-bold text-gray-500 disabled:opacity-40 bg-white cursor-pointer shadow-2xs">
              Next →
            </button>
          </div>
        )}
