...
export default function ProductDetailsPage() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetSingleProductForCustomer(id as string);
  const { brandsData } = useBrands();
  ...
  // Local State Management
  const [userSelectedShade, setUserSelectedShade] =
    useState<ProductShade | null>(null);
  const [userSelectedImg, setUserSelectedImg] = useState<string>("");
    ...
    return (
      <div className="min-h-screen bg-[#FAF9F6] flex flex-col items-center justify-center gap-4">
        ...
        <button
          ...
          className="bg-[#1A2E22] text-white px-6 py-2 rounded-xl text-xs font-bold"
        >
          Back to Shop
        </button>
      </div>
  ...
  const defaultShade =
    product.shades && product.shades.length > 0
      ? product.shades.find((s: any) => s.status === "Active") ||
        product.shades[0]
      : null;
  ...
  const defaultImg = selectedShade?.shadeImage
    ? selectedShade.shadeImage
    : product.commonImages && product.commonImages.length > 0
  ...
  const isFavorite =
    product._id && Array.isArray(wishlistItems)
      ? wishlistItems.some(
          (item: any) =>
  ...
  const categoryId =
    typeof product.category === "object" ? (product.category as any)?._id : "";
  const categoryName =
  ...
  const handleCategoryNavigation = (
    type: "category" | "subCategory" | "itemName" | "brand",
    value: string,
  ) => {
    if (!value) return;
    let queryKey = type as string;
  ...
  return (
    <div className="min-h-screen pt-28 pb-16 px-4 md:px-12 text-[#2C3E35]">
      <div className="container mx-auto p-6 md:p-10 rounded-[32px]">
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider opacity-60 hover:opacity-100 mb-8 transition-opacity"
        >
          <ArrowLeft size={14} /> Back to Shop
        </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-12">
          ...
          <div className="flex flex-col justify-between space-y-6 overflow-visible">
            <div className="space-y-4">
              <h1 className="text-xl md:text-2xl font-sans font-semibold text-gray-800 tracking-tight">
                {product.name}
              </h1>
              <p className="text-xs text-gray-400 font-medium">
                Size: {product.weightOrVolume} {product.unit}
              </p>
              ...
              <div className="pt-1">
                {isOutOfStock ? (
                  <div className="inline-flex items-center gap-2 bg-red-50 border border-red-200 px-4 py-1.5 rounded-xl">
                    <span className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse" />
                    <span className="text-sm md:text-base font-bold text-red-600 uppercase tracking-wide">
                      Out of Stock
                    </span>
                  ...
                  <div className="inline-flex items-center gap-2 bg-emerald-50 border border-emerald-200 px-4 py-1.5 rounded-xl">
                    <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="text-sm md:text-base font-bold text-emerald-600 uppercase tracking-wide">
                      In Stock
                    </span>
                    <span className="text-xs text-emerald-600 font-bold font-mono">
                      ({currentAvailableStock} items left)
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center flex-wrap gap-2.5 pt-1">
                <span className="text-xl font-bold text-[#E92C66]">
                ...
                {discountAmount > 0 && (
                  <>
                    ...
                    <span className="bg-[#612193] text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
              ...
              <div className="flex items-center gap-2 pt-1 flex-wrap">
                <span className="bg-[#121B2B] text-white text-[11px] px-3 py-1 rounded-full font-medium">
                  No #{product.salesCount || 1}{" "}
                  {product.promotion || "Best Seller"}
                </span>
                <span className="text-xs text-gray-400 font-medium">
                  in{" "}
                  <span
                    ...
                    className="text-[#E92C66] underline font-semibold cursor-pointer uppercase"
                  >
                    {product.subCategory}
                  </span>
                </span>
              ...
              {product.shades && product.shades.length > 0 && (
                <div className="pt-2">
                  <h3 className="text-xs font-bold text-gray-700 mb-2">
                    Select Shade:{" "}
                    <span className="text-[#E92C66] ml-1 font-semibold">
                      {selectedShade?.shadeName || "None"}
                    </span>
                  </h3>
                    ...
                    {product.shades.map((shade: ProductShade, idx: number) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-8 h-8 rounded-full border transition-all relative ${isShadeSelected ? "ring-2 ring-offset-2 ring-gray-800 scale-105" : "border-gray-200 hover:scale-105"}`}
                          style={{
                            backgroundColor: shade.shadeColorCode || "#ccc",
                          }}
                          title={`${shade.shadeName} (${shade.stock} left)`}
                        >
                          {shade.stock === 0 && (
                            <div className="absolute inset-0 bg-white/0 rounded-full flex items-center justify-center text-[10px] font-bold text-red-600 shadow-inner">
                              X
                            </div>
                          )}
                        </button>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center border border-gray-200 rounded-lg bg-white shadow-sm overflow-hidden h-[44px]">
                  ...
                  <span className="w-9 text-center text-xs font-bold text-gray-800">
            ...
            {/* 🎯 BRIEF DESCRIPTION (শব্দ ভাঙা ১০০% চিরতরে বন্ধ) */}
            <div className="pt-2 border-t border-gray-100 w-full">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">
                Brief Description
              </p>
              <div
                className={`text-xs text-gray-600 leading-relaxed whitespace-normal tracking-normal hyphens-none [word-break:normal] [overflow-wrap:anywhere] break-words ${
                  !isDescExpanded
                    ? "line-clamp-3 text-ellipsis overflow-hidden"
                    : ""
                }`}
                style={{ wordBreak: "normal", overflowWrap: "break-word" }} // inline CSS দিয়ে ব্রাউজারকে বাধ্য করা হলো
                ...
                <div className="text-gray-800 font-medium">
                  ...
                  {categoryName && (
                    <span
                      ...
                      className="hover:text-[#E92C66] hover:underline cursor-pointer transition-colors capitalize"
                    >
                      {categoryName}
                    </span>
                  )}
                  ...
                  {product.itemName && (
                    <>
                      ...
                      <span
                        onClick={() =>
                          handleCategoryNavigation("itemName", product.itemName)
                        }
                        className="hover:text-[#E92C66] hover:underline cursor-pointer transition-colors capitalize"
                      >
                        {product.itemName}
          ...
          {activeTab === "desc" && (
            <div className="w-full">
              <div
                ...
                className="prose max-w-none text-xs text-gray-600 leading-relaxed whitespace-normal tracking-normal hyphens-none [word-break:normal] [overflow-wrap:anywhere] break-words"
                style={{ wordBreak: "normal", overflowWrap: "break-word" }}
              />
              <p className="text-[11px] font-bold text-gray-800 mt-4">
            ...
            <div className="w-full">
              <div
                dangerouslySetInnerHTML={{
                  __html:
                    product.howToUse ||
                    "<p>Apply smoothly over wet body structure skin surface.</p>",
                }}
                className="prose max-w-none text-xs text-gray-600 leading-relaxed whitespace-normal tracking-normal hyphens-none [word-break:normal] [overflow-wrap:anywhere] break-words"
                style={{ wordBreak: "normal", overflowWrap: "break-word" }}
              />
