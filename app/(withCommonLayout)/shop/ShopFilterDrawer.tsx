...
export default function ShopFilterDrawer({
  ...
  const handleTouchEnd = () => {
    ...
    // ১. বাম থেকে ডানে সোয়াইপ করলে ওপেন হবে
    if (!isOpen && swipeDistance > 50 && touchStartX.current < 60) {
      onOpen();
    }
    
    // ২. ডান থেকে বামে সোয়াইপ করলে ক্লোজ হবে
  ...
  return (
    <>
      {/* গ্লোবাল সোয়াইপ ডিটেকশন কন্টেইনার */}
      <div 
        ...
        className="lg:hidden"
      >
        {/* 🏷️ বাম পাশের স্টিকি ভাসমান ট্যাব */}
        {!isOpen && (
          <div 
            onClick={onOpen}
        ...
        {/* 📱 ড্রয়ার বডি - বাম দিক থেকে এন্ট্রি */}
        <div
                                    ...
                                    return (
                                      <li
                                        ...
                                        onClick={() => {
                                          ...
                                          onClose(); // 🎯 ফিক্সড: ক্যাটাগরি আইটেমে ক্লিক করলে এখন অটো-ক্লোজ হবে!
                                        }}
                      ...
                      <label key={finalKey} className="flex items-center justify-between cursor-pointer group select-none text-slate-600 hover:text-slate-900 py-0.5">
                        <div className="flex items-center gap-3">
                          <input
                            ...
                            onChange={() => toggleFilter(selectedBrands, setSelectedBrands, finalKey)} // 🎯 ইউজার নিজে অফ করবেন, অটো-ক্লোজ হবে না
                ...
                {(["Normal", "Oily", "Dry", "Combination", "Sensitive"] as SkinType[]).map((type) => (
                  <label key={type} className="flex items-center gap-3 cursor-pointer py-0.5">
                    <input
                      ...
                      onChange={() => toggleFilter(selectedSkinTypes, setSelectedSkinTypes, type)} // 🎯 অটো-ক্লোজ হবে না
                ...
                {[5, 4, 3, 2, 1].map((stars) => (
                  <label key={stars} className="flex items-center gap-3 cursor-pointer text-sm py-0.5">
                    <input
                      ...
                      onChange={() => toggleFilter(selectedRatings, setSelectedRatings, stars)} // 🎯 অটো-ক্লোজ হবে না
                      className="w-4 h-4 rounded accent-[#2D4A3E]"
                    />
                ...
                {(["New Arrivals", "Best Sellers", "Trending"] as PromotionTag[]).map((promo) => (
                  <label key={promo} className="flex items-center gap-3 cursor-pointer py-0.5">
                    <input
                      ...
                      onChange={() => toggleFilter(selectedPromotions, setSelectedPromotions, promo)} // 🎯 অটো-ক্লোজ হবে না
                      className="w-4 h-4 rounded accent-[#2D4A3E]"
                    />
                    {promo}
                  </label>
