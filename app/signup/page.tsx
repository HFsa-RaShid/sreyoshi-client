...
export default function SignUpPage() {
  ...
  // অলরেডি লগইন থাকলে হোম পেজে পাঠিয়ে দাও
  useEffect(() => {
    if (status === "authenticated" || session) {
      router.replace("/");
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 🎯 ফ্রন্টএন্ড পাসওয়ার্ড ম্যাচ চেকিং
    if (password !== confirmPassword) {
      setError("Password and Confirm Password do not match!");
      return;
    }
    ...
    try {
      ...
      // 🎯 ব্যাকএন্ডের রেজিস্ট্রেশন API-তে রিকোয়েস্ট
      const res = await fetch(
        "https://sreyoshi-server.vercel.app/api/v1/auth/register",
        {
          method: "POST",
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 font-sans pt-24 bg-gray-50/50 pb-12">
      <div className="max-w-md w-full bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
        ...
        {error && (
          <p className="text-xs text-rose-600 bg-rose-50 p-3 rounded-xl mb-4 text-center font-medium border border-rose-100/70">
            {error}
          </p>
        )}

        {/* পাসওয়ার্ডের রুলস ইউজারের সুবিধার জন্য একটু নিচে হিন্টস দিয়ে দেওয়া ভালো */}
        <p className="text-[11px] text-gray-400 text-center mb-4 bg-gray-50 p-2 rounded-lg">
          💡 Password rules: Min 6 chars, 1 uppercase, 1 lowercase & 1 number.
        </p>
